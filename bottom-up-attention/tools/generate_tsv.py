    ...
    if split_name == 'coco_test2014':
      ...
      with open('/data/visualgenome/image_data.json') as f:
        for item in json.load(f):
          ...
          split.append((filepath,image_id))      
    elif split_name.startswith('textvqa'):
        if split_name.endswith('train'):
            img_prefix = 'data/TextVQA/train_images'
            with open('data/TextVQA/TextVQA_0.5_train.json') as f:
                data = json.load(f)['data']
                for item in data:
                    qid = item['question_id']
                    image_id = item['image_id']
                    filepath = os.path.join(img_prefix, "{}.jpg".format(image_id))
                    split.append((filepath, qid))
        elif split_name.endswith('val'):
            img_prefix = 'data/TextVQA/val_images'
            with open('data/TextVQA/TextVQA_0.5_val.json') as f:
                data = json.load(f)['data']
                for item in data:
                    qid = item['question_id']
                    image_id = item['image_id']
            ...
            img_prefix = 'data/TextVQA/test_images'
            with open('data/TextVQA/TextVQA_0.5_test.json') as f:
                data = json.load(f)['data']
                for item in data:
                    qid = item['question_id']
                    image_id = item['image_id']
